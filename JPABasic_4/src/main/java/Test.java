...
// HashMap 옵션, remove, persist & find
// 이전 JPABasic_3 의 Test.java 그대로 복사 후 불필요한 comment 및 코드 정리
// 진행 순서
// 1. show_sql true 로 하고 이전 Test.java 하나씩 다시 실행하면서 sql 확인
// 2. ddl.auto create, update 로 하고 모든 코드는 코멘트 한 뒤 비교 테스트
// 3. remove
// 4. persist & remove
public class Test {
...
//		{
//			Employee e = em.find(Employee.class, 2); // id 가 1
...
//			em.merge(e); // 영속화 ( 이 시점에 insert 되지 않는다. )			
//			// Hibernate: select e1_0.id,e1_0.address,e1_0.name from employee e1_0 where e1_0.id=?
//			// Hibernate: update employee set address=?,name=? where id=?
//		}		
		...
		// find() 는 대상이 이미 영속확 되어 있으면 테이블에서 조회 X
//		{
//			Employee e = new Employee();
//			e.setId(3);
//			e.setName("삼길동");

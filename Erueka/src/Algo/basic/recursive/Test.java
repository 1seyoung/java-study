...
//		System.out.println("m1() " + i++);
    ...
    static void m3() {
        ...
        m3_cnt--;
        m3();
        m3_cnt++; // 이 코드가 없으면  static 변수 m3_cnt 가 0 으로 감소한 후 계속 0 유지, 원복 필요
...
//		m4_cnt--;
//		m4(m4_cnt);
//		m4_cnt++; // 이 코드가 없으면  static 변수 m4_cnt 가 0 으로 감소한 후 계속 0 유지, 원복 필요

        m4(m4_cnt - 1); // 호출 시 전달받은 m4_cnt 는 변화 X

//		m4(m4_cnt--); // 5 가 전달 => 스택오버플로우 발생

//		m4(--m4_cnt); // 호출 시 전달받은 m4_cnt 가 1 감소
//		m4_cnt++; // 원복 필요

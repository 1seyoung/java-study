...
//   cycle 이 발생하는 간선 skip <= union-find 알고리즘 사용
//   선택한 간선의 수가 정점의 수 - 1 개 될 때 까지 진행
    ...
    public static void main(String[] args) throws Exception{
        ...
        for (int i = 0; i < edgesLen; i++) {
            ...
            if( union(edge.v1, edge.v2 )) {
                ...
                sum += edge.c; // 전체 MST 비용 추가
                cnt++;
                if( cnt == V - 1 ) break; // 모든 간선을 선택했으면
            }
        }
...
//	}
    ...
    // findSet(v1), findSet(v2) 중복 실행이 되므로  쿠르스칼에 맞게 변형

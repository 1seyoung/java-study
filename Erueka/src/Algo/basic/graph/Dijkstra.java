...
public class Dijkstra {
  ...
  static int V, start, end; 	// sum : MST 비용
  ...
  public static void main(String[] args) throws Exception{
    ...
    while( ! pqueue.isEmpty() ) {
      ...
      Vertex vertex = pqueue.poll(); // vertex = 교재 392 하단 그림의 A 에 해당

      // visit 체크 (별도의 visit[] 이용해서 풀이 가능)
      // cost[] 갱신여부 비용비교를 통해서 처리
      // 이미 방문 그리고 비용이 더 높기때문에 고려할 필요 X
      if( cost[vertex.v] < vertex.c ) continue;

      // 시작정점 (start) ~~> vertex.v 로부터 cost 비용이 갱신될 수 있다.
      for (int i = 0; i < V; i++) {
        // 꺼낸 정점으로부터 갈 수 있는 정점을 고려
        if( matrix[vertex.v][i] == 0 ) continue;

        // i = 교재 392 하단 그림의 E 에 해당 ( B, C, E 를 고려할 때
        // start 에서 i 경유해서 가는 비용이 원 비용 (cost[i]) 보다 낮으면 cost[i] 비용을 갱신하고 다시 경쟁하도록 pqueue 에 담는다.
  ...
  static class Vertex{
    int v, c; // v : 시작정점에서부터 갈 수 있는 정점, c : 시작정점에서부터 v 정점까지의 최소 비요
    Vertex(int v, int c){
      this.v = v;
      this.c = c;
    }
...
//visit 없으면 어떻게 되는지 확인
// 오래 걸림 왜냐면 이미 방문한 정점을 다시 PQ 에 넣기 때문에
